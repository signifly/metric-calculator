...
export class ShopifyClient {
  ...
  async getMetrics(days: number = 30): Promise<LiveMetrics> {
  ...
  private processMetrics(data: { orders: { edges: Array<{ node: { totalPriceSet: { shopMoney: { amount: string } } } }> }, customers: { edges: Array<{ node: { numberOfOrders: number } }> } }): LiveMetrics {
    const orders = data.orders.edges
    const customers = data.customers.edges

    const totalRevenue = orders.reduce((sum: number, order: { node: { totalPriceSet: { shopMoney: { amount: string } } } }) => {
      return sum + parseFloat(order.node.totalPriceSet.shopMoney.amount)
    }, 0)

    const returningCustomers = customers.filter((c: { node: { numberOfOrders: number } }) =>
