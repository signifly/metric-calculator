...
export function exportScenariosAsCSV(scenarios: Scenario[]): void {
  ...
  const records = scenarios.map((scenario) => ({
    Name: scenario.name,
    "Date Range": formatDateRange(scenario.dateRange),
    ...
    "Revenue Increase": `$${scenario.results.impact.revenueIncrease.toFixed(
      2
    )}`,
    ROI: `${scenario.results.impact.percentageIncrease.toFixed(2)}%`,
    "Break-even Months":
      scenario.results.impact.breakEvenMonths === Infinity
        ? "N/A"
