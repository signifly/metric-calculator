  ...
  try {
    ...
    if (!shop || !accessToken) {
      return NextResponse.json(
        {
          error:
            "Shopify credentials not configured. Please set SHOPIFY_STORE_DOMAIN and SHOPIFY_ADMIN_API_TOKEN in .env.local",
        },
        { status: 500 }
      );
    }
    ...
    const metrics = await client.getMetrics(startDate, endDate);
